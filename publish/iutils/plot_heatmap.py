...
def get_vmax_vmin(input_mtx, vmax_per=default_vmax_per, vmax=None, vmin=None):
    ...
    if input_mtx.has_neg:
        pos_values = matrix[np.isfinite(matrix) & (matrix > 0)]
        neg_values = matrix[np.isfinite(matrix) & (matrix < 0)]
        if pos_values.size == 0 and neg_values.size == 0:
            vmax, vmin = 1, -1
        elif pos_values.size == 0:
            vmin = np.percentile(neg_values, 100 - vmax_per)
            vmax = -vmin
        elif neg_values.size == 0:
            vmax = np.percentile(pos_values, vmax_per)
            vmin = -vmax
        else:
            vmax = np.percentile(pos_values, vmax_per)
            vmin = np.percentile(neg_values, 100 - vmax_per)
        ...
        if pos_values.size == 0:
            ...
            vmax = np.percentile(pos_values, vmax_per)
            vmin = np.percentile(pos_values, 100 - vmax_per)
