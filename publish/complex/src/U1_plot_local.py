...
def ran_get_plot_chros(input_data, n_intra_plot=4, n_inter_plot=2):
    chr_lens = input_data.row_window.chr_lens
    chros = sorted(input_data.row_window.chros, key=lambda chro: chr_lens[chro], reverse=True)
    plot_intra_chros = chros[:n_intra_plot]
